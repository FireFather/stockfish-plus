...
PolyBook polybook1;  // 1st global PolyBook instance
PolyBook polybook2;  // 2nd global PolyBook instance
...
PolyBook::PolyBook() : keycount(0), polyhash(nullptr), use_best_book_move(true), max_book_depth(255), book_depth_count(0),
	index_first(0), index_count(0), index_best(0), index_rand(0), index_weight_count(0), sr(0), last_position(0), akt_position(0),
	last_anz_pieces(0), akt_anz_pieces(0), search_counter(0), enabled(false), do_search(true)
{
}PolyBook::~PolyBook()
...
void PolyBook::set_best_book_move(const bool best_book_move)
{
    use_best_book_move = best_book_move;
}


void PolyBook::set_book_depth(const int book_depth)
{
...
Move PolyBook::probe(Position& pos)
{
    ...
    const int n = find_first_key(key);
    ...
    if (const Move m2 = pg_move_to_sf_move(pos, polyhash[idx2].move); !check_draw(m2, pos))
        return m2;
...
Key PolyBook::polyglot_key(const Position & pos) const
{
    Key key = 0;
...
Move PolyBook::pg_move_to_sf_move(const Position & pos, unsigned short pg_move) const
{
	const auto move = static_cast<Move>(pg_move);

	if (const int pt = (move >> 12) & 7)
        return make<PROMOTION>(from_sq(move), to_sq(move), static_cast<PieceType>(pt + 1));
...
int PolyBook::find_first_key(const uint64_t key)
{
    index_first = -1;
    index_count = 0;
    ...
    for (;;)
    {
	    if (const int mid = (end + start) / 2; polyhash[mid].key < key)
            start = mid;
        else
        {
...
int PolyBook::get_key_data()
{
    ...
    const uint64_t key = polyhash[index_first].key;
    ...
    const int rand_pos = (rand64() % index_weight_count);
    int weight_count = 0;
...
bool PolyBook::check_do_search(const Position & pos)
{
    ...
    const Bitboard b = akt_position ^ last_position;

    if (const int n2 = popcount(b); n2 > 6) pos_changed = true;
...
bool PolyBook::check_draw(const Move m, Position & pos)
{
    StateInfo st{};

    pos.do_move(m, st, pos.gives_check(m));
    const bool draw = pos.is_draw(64);
    pos.undo_move(m);
...
{
    uint64_t a = 0;
    auto*dst = reinterpret_cast<uint8_t*>(&a);
    const auto*src = reinterpret_cast<uint8_t*>(&d);
